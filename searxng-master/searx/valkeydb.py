...
if sys.platform != "win32":
    ...
    pwd = None  # or create a dummy replacement if needed
import logging
import warnings
...
def initialize():
    ...
    try:
        ...
        _CLIENT = None
        if sys.platform != "win32" and 'pwd' in globals():
            _pw = pwd.getpwuid(os.getuid())
            logger.exception("[%s (%s)] can't connect valkey DB ...", _pw.pw_name, _pw.pw_uid)
        else:
            logger.exception("Can't connect valkey DB ... (user info unavailable on Windows)")
    return False
