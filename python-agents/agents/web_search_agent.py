...
from config import MODELS, CHAT_DIR, SEARXNG_BASE_URL
...
class WebSearchAgent:
    ...
    async def process_tool_call(self, tool_call: Dict[str, Any], parent_chat_id: str = None) -> Dict[str, Any]:
        ...
        if not query:
            return {"agent_id":self.agent_id,"summary":"Query empty","status":"failed","error":"Empty query","timestamp":ts}

        data = await self._search_searxng(query, cat)
        with open(raw_file,"w") as f: json.dump(data,f,indent=2)
        ...
        result = {
            "agent_id": self.agent_id, "summary": summary,
            "status": status, "timestamp": ts, "raw_data_file": str(raw_file)
        }
        with open(conv_file,"w") as f: json.dump(result,f,indent=2)
        return result
    ...
    def get_status(self) -> Dict[str, Any]:
        return {"agent_id":self.agent_id,"is_available":self.is_available,"url":self.searxng_url}
