...
export async function resolveLLMFromSession(chatId: string) {
  ...
  if (!session?.providerKey || !session?.modelKey) {
    throw new Error('No provider/model set for this session. Open Settings and select a model.');
  }
  ...
  if (!modelInfo) {
    throw new Error(`Selected model '${session.modelKey}' not found in provider '${session.providerKey}'. Check Settings.`);
  }
