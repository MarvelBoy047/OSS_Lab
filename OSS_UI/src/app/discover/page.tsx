...
const Page = () => {
  ...
  return (
    // EDITED: Wrapped the entire page content in a <main> tag that controls layout and overflow.
    <main className="flex-1 flex flex-col p-6 overflow-hidden">
      ...
      <div className="flex-shrink-0">
        ...
        <hr className="border-t border-[#2C2C2C] my-4 w-full" />
        <div className="flex flex-row items-center space-x-2 overflow-x-auto pb-4">
          {topics.map((t, i) => (
            <div
              key={i}
              className={cn(
                ...
                activeTopic === t.key
                  ? 'text-cyan-accent bg-cyan-accent/30 border-cyan-accent/60'
      ...
      {/* EDITED: This div now handles its own scrolling, with the custom scrollbar. */}
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
        {loading ? (
          ...
          <div className="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4 pt-5 pb-6 w-full justify-items-center lg:justify-items-start">
            {discover &&
              discover?.map((item, i) => (
                <Link
                  href={`/?q=Summary: ${item.url}`}
                  key={i}
                  className="max-w-sm rounded-lg overflow-hidden bg-bg-secondary hover:-translate-y-[1px] transition duration-200"
                  target="_blank"
                >
                  <img
